...
int main()
{
	...
	{	//output standard
		...
		filename = (polytype[p]+"00"); //make filename
		sim >> (opath + filename + ext);	//write file out
		...
		sim << (ipath+data[p]+ext);	//read in
		lastNum = sim.PassivatedHole(holeRadius);	//make hole with r=0;
		for(int n=0; n<N; n++)//for each distinct outcome
		{
			do{
				...
				removed = sim.PassivatedHole(holeRadius);//make hole at rough center on an atom

			}while(removed==lastNum);
			lastNum = removed;	//set as new last
			//create filename
			filename = (polytype[p]+'0'+I_Str(n+1));
			//write sims to file

...
int main()
{
	...
	string I_Str(int);
	//constants
	const unsigned int N = 6;	//number of distinct outcomes
	...
	datafile << "%Si\t%C\t%H\t#Si\t#C\t#H\tDensity\tRadius\tL_X\tL_Y\tL_Z\tplytyp\telement\tNUM\n";
		...
		{*/
			...
			for(int n=0; n<N; n++)//for each distinct outcome
			{
				do{
					...
					removed = sim.PassivatedHole(holeRadius/*, sim.Center(E)*/);

				}while(removed==lastNum);
				lastNum = removed;	//set as new last
				//write sims to file
				sim >> (opath+polytype[p]+'-'+I_Str(int(holeRadius-STEP))+'-'+I_Str(sim.Atoms())+'-'+I_Str(int(sim.Density()*100))/*+'-'+I_Str(E)*/+ext);
				//write data
				for(int e=0; e<sim.elementNum; e++)	//%
					datafile << sim%e << '\t';
				for(int e=0; e<sim.elementNum; e++)	//#
				...
				for(int a=0; a<3; a++)			//lattice
					datafile << sim.lattice[a] << '\t';
				datafile << polytype[p] << '\t';	//polytype
				datafile << /*sim.element[E]*/"XXX" << '\t';	//element
				datafile << n << endl;				//NUM
				cout << p << /*',' << E << */',' << n << '\n';
			}
		//}
